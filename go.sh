...
downloadV2Ray(){
    ...
    if [[ "${DIST_SRC}" == "jsdelivr" ]]; then
        ...
        DOWNLOAD_LINK="https://github.com/v2fly/v2ray-core/releases/download/${NEW_VER}/v2ray-linux-$MACHINE.zip"
    fi
...
main(){
    ...
    local ZIPROOT="$(zipRoot "${ZIP_FILE}")"
    installSoftware unzip || return $?

    if [ -n "${EXTRACT_ONLY}" ]; then
        colorEcho ${BLUE} "Extracting V2Ray package to ${VSRC_ROOT}."

        if unzip -o "${ZIP_FILE}" -d ${VSRC_ROOT}; then
    ...
    installV2Ray "${ZIP_FILE}" "${ZIPROOT}" || return $?
    installInitScript "${ZIP_FILE}" "${ZIPROOT}" || return $?
